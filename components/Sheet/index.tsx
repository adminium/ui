...
import { SheetSize, SheetProps } from './interface';
...
function Space(baseProps: SheetProps, ref) {
  const { getPrefixCls, componentConfig, rtl } = useContext(ConfigContext);
  const props = useMergeProps<SheetProps>(baseProps, defaultProps, componentConfig?.Space);
  ...
  function getMargin(size: SheetSize) {
    if (isNumber(size)) {
      return size;
    }
    switch (size) {
...
const SpaceComponent = forwardRef<unknown, SheetProps>(Space);

SpaceComponent.displayName = 'Space';

export default SpaceComponent;

export { SheetSize, SheetProps };
